...
class AIProcessor: ObservableObject {
    ...
    func enhanceNotes(userNotes: String, transcript: String, template: NoteTemplate) async throws -> String {
        ...
        do {
            let query = ChatQuery(
                messages: [
                    ChatQuery.ChatCompletionMessageParam(role: .system, content: "You are an AI assistant that helps improve meeting notes. You analyze meeting transcripts and user notes to create comprehensive, well-structured meeting summaries.")!,
    ...
    func answerQuestion(question: String, transcript: String, chatHistory: [ChatMessage] = []) async throws -> String {
        ...
        var messages: [ChatQuery.ChatCompletionMessageParam] = [
            ChatQuery.ChatCompletionMessageParam(role: .system, content: """
                ...
                """)!,
            ChatQuery.ChatCompletionMessageParam(role: .user, content: """
                MEETING TRANSCRIPT:
                \(transcript)
        ...
        for message in chatHistory.suffix(10) { // Keep last 10 messages for context
            messages.append(ChatQuery.ChatCompletionMessageParam(
        ...
        messages.append(ChatQuery.ChatCompletionMessageParam(role: .user, content: question)!)
    ...
    func generateFollowUpEmail(transcript: String, enhancedNotes: String) async throws -> String {
        ...
        do {
            let query = ChatQuery(
                messages: [
                    ChatQuery.ChatCompletionMessageParam(role: .system, content: "You are a professional assistant helping to draft follow-up emails after meetings.")!,
                    ChatQuery.ChatCompletionMessageParam(role: .user, content: prompt)!
                ],
                model: .gpt4,
                temperature: 0.2
...
extension ChatQuery.ChatCompletionMessageParam.Role {
